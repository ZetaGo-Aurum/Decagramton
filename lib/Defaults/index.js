...
console.log(chalk.green.bold('║           ZETAGO-AURUM - DECAGRAMTON       ║'));
console.log(chalk.green.bold('║        The Ten-Pointed Star of Speed       ║'));
console.log(chalk.green.bold('╚════════════════════════════════════════════╝'));
console.log(chalk.cyan('Running version... checking updates...'));

// AUTO UPDATE CHECK
(async () => {
    try {
        const fetch = require('node-fetch');
        const res = await fetch('https://registry.npmjs.org/@zetagoaurum-socket/decagramton/latest');
        const data = await res.json();
        const latest = data.version;
        // Simple log, logic to compare can be added if needed
        console.log(chalk.blue(`Latest Version on NPM: ${latest}`));
    } catch (e) {
        // Silent fail
    }
})();
